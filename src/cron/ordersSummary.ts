...
async function ordersSummary() {
    ...
    const defaultMailAddress   = 'contact.tumeplay@fabrique.social.gouv.fr';
    ...
    for( const orderIndex in orders )
    {
		...
		const contact  = ( supports.length > 0 ? supports[0] : '' );
	...
	for( const orderKey in parsedOrders )
	{
    	...
    	if( parsedOrder.mail )
    	{
			...
			await Container.get(MailerService).send('contact.tumeplay@fabrique.social.gouv.fr', 'Récapitulatif de commandes', 'orders_summary_user', variables);	
    	}
	}
	...
	if( orders.length > 0 )
	{
    	...
    	await Container.get(MailerService).send('contact.tumeplay@fabrique.social.gouv.fr', 'Récapitulatif de commandes', 'orders_summary', variables);
	}
    return;    
}
