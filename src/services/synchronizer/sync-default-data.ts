...
export default class SyncDefaultData {
    ...
    public async syncDefaultThemes(): Promise<{ success: boolean }> {
        try {
            ...
            await Promise.all(
                themesToCreate.map(async themeObj => {
                    try {
    ...
    public async syncDefaultCategories(): Promise<{ success: boolean }> {
        ...
        await Promise.all(
            categoriesToCreate.map(async categoryObj => {
    ...
    public async syncDefaultQuestions(): Promise<{ success: boolean }> {
        ...
        let questionsToCreate: IQuestionContentDTO[] = defaultQuestionList.map(
            (questionItemTemp: IQuestionContentForDefault) => {
                let category = questionCategories.find(e => {
                    return e.id === questionItemTemp.category;
                });
                let theme = thematiques.find(e => {
                    return e.id === questionItemTemp.theme;
                });
                if (!category)
                    console.error('No category found ( by id Fictitious ) for this question:', questionItemTemp);
                return {
                    ...
                    defaultPicturePath:
                        questionItemTemp.picture && questionItemTemp.picture !== 'undefined'
                            ? questionItemTemp.picture
                            : null,
                };
            },
        );
        ...
        await Promise.all(
            questionsToCreate.map(async questionObj => {
                try {
                    ...
                    let picture = await this.generatePicture(
                        questionObj,
                        'quizz',
                        'uploads/pictures/question',
                        questionObj.defaultPicturePath,
                    );
    ...
    public async syncDefaultContents(): Promise<{ success: boolean }> {
        ...
        // Checking if question category already exist
        await Promise.all(
            contentsToCreate.map(async contentObject => {
                try {
                    let pictureModelService = Container.get('pictureModel');
                    let category = questionCategories.find(e => {
                        return e.id === contentObject.categoryId;
                    });
                    let theme = thematiques.find(e => {
                        return e.id === contentObject.themeId;
                    });

                    if (!category || !theme) {
                        this.logger.error(
                            'No category / theme spotted : ' + contentObject.themeId + ' | ' + contentObject.categoryId,
                        );
                    }
                    ...
                    let picture = await this.generatePicture(
                        contentObject,
                        'content',
                        'uploads/pictures/content',
                        contentObject.picture,
                    );
            ...
            productsToCreate.map(async productToCreate => {
                try {
                    ...
                    let picture = await this.generatePicture(
                        productToCreate,
                        'product',
                        'uploads/pictures/product',
                        productToCreate.picture,
                    );
    ...
    public async syncDefaultBoxs(): Promise<{ success: boolean }> {
        ...
        // Checking if question category already exist
        await Promise.all(
            boxsToCreate.map(async boxToCreate => {
                try {
                    let pictureModelService = Container.get('pictureModel');
