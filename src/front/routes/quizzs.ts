...
export default (app: Router) => {
	...
	route.post(QUIZZ_QUESTION_ROOT + '/add', middlewares.isAuth, uploadCategory.single('questionContentPicture'), async (req: Request, res: Response) => {
	...
	route.post(QUIZZ_QUESTION_ROOT + '/edit/:id', middlewares.isAuth, uploadCategory.single('questionContentPicture'), async (req: Request, res: Response) => {
		const logger: any = Container.get('logger');
		try {
			const documentId = +req.params.id;
	...
	*/
	...
	route.get(QUIZZ_CATEGORY_ROOT + '/', middlewares.isAuth, async (req: Request, res: Response) => {
		try {
			...
			const categories: IQuestionCategory[] = await questionCategories.findAll({ include: ['picture', 'itsTheme'] });
			/**
			 * TODO :
	...
	route.post(QUIZZ_CATEGORY_ROOT + '/add', middlewares.isAuth, uploadCategory.single('categoryPicture'),
	...
	route.post(QUIZZ_CATEGORY_ROOT + '/edit/:id', middlewares.isAuth, uploadCategory.single('categoryPicture'),
